...
// Combine filters into one WHERE clause for correctness
WHERE 
    // Location Filter
...
CALL (pt) {
    ...
    // Try to find at least one cert to calculate experience, even if no search term
...
// --- 6. Sorting & Limiting (PERFORMANCE OPTIMIZATION) ---
// We limit HERE, before fetching extra details for the final result
ORDER BY partial_score DESC, distance_in_meters ASC
LIMIT coalesce($limit, 10)

// --- 7. Hydrate Details for the Top Results ---
// Now we only fetch "all certificates" for the top 10 users, not the whole database
...
RETURN 
    ...
    query_results = await asyncio.to_thread(
        ...
        query, 
        {
            ...
            "min_fuzzy_scoring": 1, 
            "lat": state.user_origin.latitude, 
            "lon": state.user_origin.longitude, 
